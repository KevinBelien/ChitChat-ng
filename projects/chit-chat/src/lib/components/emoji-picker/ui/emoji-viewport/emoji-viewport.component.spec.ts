...
describe('EmojiViewportComponent', () => {
	...
	it('should prevent default scroll behavior and use custom scroll step on wheel event', () => {
		const preventDefaultSpy = jest.spyOn(
			WheelEvent.prototype,
			'preventDefault'
		);
		const handleWheelScrollSpy = jest.spyOn(
			component as any,
			'handleWheelScroll'
		);
		const scrollToOffsetSpy = jest.spyOn(
			cdkViewport,
			'scrollToOffset'
		);
		const scrollEvent = new WheelEvent('wheel', { deltaY: 100 });

		viewportDebugElement.triggerEventHandler('wheel', scrollEvent);
		fixture.detectChanges();

		expect(handleWheelScrollSpy).toHaveBeenCalledWith(scrollEvent);
